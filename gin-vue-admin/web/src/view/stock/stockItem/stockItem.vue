    ...
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline"
        ...
        <el-form-item label="关联分类" prop="stockCategoryId">
          <el-select v-model="searchInfo.stockCategoryId" filterable placeholder="请选择关联分类" :clearable="true">
    ...
    <div class="gva-table-box">
      <div class="gva-btn-list">
        ...
        <el-button v-auth="btnAuth.batchDelete" icon="delete" style="margin-left: 10px;"
                   :disabled="!multipleSelection.length" @click="onDelete">删除
        </el-button>
      ...
      >
        ...
        <el-table-column align="left" label="关联分类" prop="stockCategoryId" width="120">
          <template #default="scope">
            <span>{{ filterDataSource(dataSource.stockCategoryId, scope.row.stockCategoryId) }}</span>
          </template>
        </el-table-column>
        <el-table-column align="left" label="排序(越小越靠前)" prop="sort" width="120" />

        <el-table-column align="left" label="股票名字" prop="name" width="120" />

        <el-table-column align="left" label="股票代码" prop="code" width="120" />

        <el-table-column align="left" label="公司名称" prop="companyName" width="120" />

        <el-table-column align="left" label="公司简介" prop="companyProfile" width="120" />

        <el-table-column align="left" label="所属行业" prop="industry" width="120" />

        <el-table-column align="left" label="上市日期" prop="listingDate" width="180">
          <template #default="scope">{{ formatDate(scope.row.listingDate) }}</template>
        </el-table-column>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
          <template #default="scope">
            ...
            <el-button v-auth="btnAuth.edit" type="primary" link icon="edit" class="table-button"
                       @click="updateStockItemFunc(scope.row)">编辑
            </el-button>
            <el-button v-auth="btnAuth.delete" type="primary" link icon="delete" @click="deleteRow(scope.row)">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-drawer destroy-on-close :size="appStore.drawerSize" v-model="detailShow" :show-close="true"
      ...
      <el-descriptions :column="1" border>
        <el-descriptions-item label="关联分类">
          <template #default="scope">
            <span>{{ filterDataSource(dataSource.stockCategoryId, detailForm.stockCategoryId) }}</span>
          </template>
        </el-descriptions-item>
        <el-descriptions-item label="排序(越小越靠前)">
          {{ detailForm.sort }}
        </el-descriptions-item>
        <el-descriptions-item label="股票名字">
          {{ detailForm.name }}
        </el-descriptions-item>
        ...
        <el-descriptions-item label="公司简介">
          {{ detailForm.companyProfile }}
        </el-descriptions-item>
        <el-descriptions-item label="所属行业">
          {{ detailForm.industry }}
        </el-descriptions-item>
        <el-descriptions-item label="上市日期">
          {{ detailForm.listingDate }}
        </el-descriptions-item>
...
<script setup>
  ...
  const getTableData = async () => {
    const table = await getStockItemList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
    if (table.code === 0) {
  ...
  const onDelete = async () => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
      ...
      const res = await deleteStockItemByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
  ...
  // 更新行
  const updateStockItemFunc = async (row) => {
    const res = await findStockItem({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
  ...
  // 删除行
  const deleteStockItemFunc = async (row) => {
    const res = await deleteStockItem({ ID: row.ID })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功'
