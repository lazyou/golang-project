      ...
      >
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
          <template #default="scope">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateStockCategoryFunc(scope.row)">
              编辑
            </el-button>
...
<script setup>
  import {
    createStockCategory,
    deleteStockCategory,
    deleteStockCategoryByIds,
    updateStockCategory,
    findStockCategory,
    getStockCategoryList
  } from '@/api/stock/stockCategory'
  ...
  const getTableData = async () => {
    const table = await getStockCategoryList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
    if (table.code === 0) {
  ...
  const onDelete = async () => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
      ...
      const res = await deleteStockCategoryByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
  ...
  // 更新行
  const updateStockCategoryFunc = async (row) => {
    const res = await findStockCategory({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
  ...
  // 删除行
  const deleteStockCategoryFunc = async (row) => {
    const res = await deleteStockCategory({ ID: row.ID })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功'
  ...
  const enterDialog = async () => {
    ...
    elFormRef.value?.validate(async (valid) => {
      ...
      switch (type.value) {
        case 'create':
          res = await createStockCategory(formData.value)
          break
        case 'update':
          res = await updateStockCategory(formData.value)
          break
        default:
          res = await createStockCategory(formData.value)
          break
